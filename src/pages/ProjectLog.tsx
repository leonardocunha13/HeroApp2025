...
const ProjectLog = () => {
        ...
        data.flatMap(async (form) => {
          const equipmentData = await Promise.all(
            form.equipmentDetails.map(async (equipment) => {
              ...
              const equipmentTagID = await getEquipmentTagID(
              ...
              const content = await getContentByFormIDandTagID(
                form.formID,
                equipmentTagID,
              );
  ...
  const handleResumeTest = async () => {
    ...
    try {
      ...
      const equipmentTagID = await getEquipmentTagID(
        selectedForm.equipmentName,
        selectedForm.equipmentTag,
      );
            ...
            <div key={index} style={{ maxWidth: "100%" }}>
              ...
              >
                ...
                <Text>
                  ...
                  <span
                    style={{
                      color: selectedForm?.submission === 1 ? "green" : "red",
                    }}
                  >
                    {selectedForm?.submission === 1
                      ? "Test Finished"
                      : "Test not Finished"}
                  </span>
                </Text>
      ...
      <Dialog.Root open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        ...
        <Dialog.Content className="DialogContent">
          ...
          <Dialog.Description asChild>
            ...
            >
              <Text>
                <strong>Project Name:</strong> {selectedForm?.projectName}
              </Text>
              <Text>
                <strong>Form Name:</strong> {selectedForm?.formName}
              </Text>
              <Text>
                <strong>Equipment:</strong> {selectedForm?.equipmentName}
              </Text>
              <Text>
                <strong>Tag:</strong> {selectedForm?.equipmentTag}
              </Text>
            </div>
          </Dialog.Description>
